...
public class KakaoLoginController {
  ...
  @Autowired
  public KakaoLoginController(UserService userServices, OAuthUserService oAuthService,
  ...
  public ResponseEntity<String> kakaoLoginCallBack(HttpServletRequest req, HttpServletResponse res,
    ...
    try {
      ...
      OAuthTokenDto oAuthToken = mapper.readValue(responseBody, OAuthTokenDto.class);
      ...
      Optional<OAuth> optionalOAuthDto = getOAuthDtoByoAuthid(oAuthid);
      ...
      if (!optionalOAuthDto.isPresent()) {
        ...
        Optional<UserCreateRequestDto> optionalUserDto = getUserDtoByEmail(email);
        OAuth oAuthDto = new OAuth(kakaoUserInfoResponseDto);
      ...
      OAuth oAuthDto = optionalOAuthDto.get();
      ...
      // 4-2-2. 회원 탈퇴 했다가 다시 가입한 유저인 경우
      if(oAuthDto.getStatus().name().equals(OAuth.Status.STOP.name())) {
  ...
  private Optional<OAuth> getOAuthDtoByoAuthid(String oAuthid) throws SQLException {
    try {
      OAuth oAuthDto = oAuthService.getOAuthByOAuthId("KAKAO", oAuthid);
  ...
  private Optional<UserCreateRequestDto> getUserDtoByEmail(String email) throws SQLException {
    try {
  ...
  private OAuthTokenDto updateToken(OAuthTokenDto oAuthToken) throws URISyntaxException, IOException{
