...
//  1
  public SimpleEstimateResponseDto getMySpemBySpemId(int spemId, int userId) {
    ...
    return Optional.of(spem)
        .filter(e -> e.getUser().getUserSeq() == userId)
...
//  //1
//  @Transactional
//  public SimpleEstimateResponseDto updateMySpem(int spemId, SimpleEstimateRequestDto spemRequestDto, int userId) {
//    
//    SimpleEstimate spem = getAuthorizedSpem(spemId, userId);
//    
//    spem.setPhone(spemRequestDto.phone());
//    spem.setCleaningService(spemRequestDto.cleaningService());
//    spem.setRegion(spemRequestDto.region());
//    spemRepository.flush();
//    
//    return SimpleEstimateResponseDto.fromEntity(
//        getSpemOrThrow(spem.getSimpleEstimateSeq()),
//        obfuscator);
//  }
//  
//  //2
//  @Transactional
//  public SimpleEstimateResponseDto updateSpemByAuth(int spemId, SimpleEstimateRequestDto spemRequestDto, String phone) {
//    
//    verificationService.validateVerify(phone);
...
//  2
  ...
  private SimpleEstimate getAuthorizedSpem(int spemId, int userId) {
    ...
    if (spem.getUser().getUserSeq() != userId) {
